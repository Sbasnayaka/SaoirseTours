...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($rev['id'])) {
        $stmt = $pdo->prepare("INSERT INTO testimonials (name, rating, review, source, link, image) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$name, $rating, $review, $source, $link, $image]);
    } else {
        $stmt = $pdo->prepare("UPDATE testimonials SET name=?, rating=?, review=?, source=?, link=?, image=? WHERE id=?");
        $stmt->execute([$name, $rating, $review, $source, $link, $image, $rev['id']]);
    }
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="name" value="<?php echo htmlspecialchars($rev['name']); ?>" required>
                </div>
                
                <div class="mb-3">
                    <label class="form-label">Review Source</label>
                    <select class="form-select" name="source">
                        <option value="direct" <?php echo ($rev['source'] ?? '') == 'direct' ? 'selected' : ''; ?>>Direct / Social</option>
                        <option value="tripadvisor" <?php echo ($rev['source'] ?? '') == 'tripadvisor' ? 'selected' : ''; ?>>TripAdvisor</option>
                        <option value="google" <?php echo ($rev['source'] ?? '') == 'google' ? 'selected' : ''; ?>>Google Reviews</option>
                        <option value="facebook" <?php echo ($rev['source'] ?? '') == 'facebook' ? 'selected' : ''; ?>>Facebook</option>
                    </select>
                </div>

                <div class="mb-3">
                    <label class="form-label">Verification Link (Optional)</label>
                    <input type="url" class="form-control" name="link" placeholder="https://..." value="<?php echo htmlspecialchars($rev['link'] ?? ''); ?>">
                </div>

                <div class="mb-3">
                    <label class="form-label">Review Text</label>
                    <textarea class="form-control" name="review" rows="4" required><?php echo htmlspecialchars($rev['review']); ?></textarea>
                </div>
            </div>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body text-center">
                        <?php if (!empty($rev['image'])): ?>
                            <img src="../uploads/<?php echo htmlspecialchars($rev['image']); ?>" class="img-fluid rounded-circle mb-3" style="width: 100px; height: 100px; object-fit: cover;">
                        <?php else: ?>
